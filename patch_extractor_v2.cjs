...
const replacement = `export function mapExtractedToFormFields(raw: any): Partial<ExtractedProduct> {
  ...
  // Flatten the entire raw object into a single-level dictionary for foolproof key matching
  ...
  // Extremely fuzzy key matcher that looks anywhere in the flattened JSON
  ...
  // 4. Parse Advanced Sections (Extract from the original unflattened object)
